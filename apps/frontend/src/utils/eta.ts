...
export function calcETA(item: StudyItem): ETAResult {
  ...
  if (remaining <= 0) return { daysLeft: 0, onTrack: true, etaDate: new Date() }
  ...
  if (avg <= 0) return { daysLeft: Infinity, onTrack: false, etaDate: new Date(9999, 0) }
...
export function pct(item: StudyItem): number {
  ...
  return Math.min(100, Math.round((item.currentProgress / item.totalScope) * 100))
}
