    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary, #EDEEEC)', lineHeight: 1.3 }}>
            {item.title}
          </div>
          <div style={{ fontSize: 12, color: 'var(--text-muted, #8B929A)', marginTop: 4, fontFamily: "'JetBrains Mono', monospace" }}>
        ...
        <div style={{ flexShrink: 0 }}>
          <CircularProgressDial percentage={p} onTrack={onTrack} size={56} strokeWidth={5} />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, gap: 8 }}>
        <span style={{ fontSize: 11, fontWeight: 500, color: rhythmColor, background: rhythmBg, padding: '4px 10px', borderRadius: 99 }}>
          {rhythmText}
        </span>
        {item.status !== 'done' && (
          <button
