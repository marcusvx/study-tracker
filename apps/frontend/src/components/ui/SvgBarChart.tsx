      ...
      {data.map((d, i) => {
        ...
        return (
          ...
          >
            ...
            {isHov && (
              <g>
                <rect x={x - 22} y={y - 28} width={44} height={20} rx={4} fill="#1E2226" stroke="#2D3339" strokeWidth={1} />
                <text x={x} y={y - 14} textAnchor="middle" fill="#EDEEEC" fontSize={11} fontFamily="'JetBrains Mono', monospace">
                  {d.progresso}
                </text>
              </g>
            )}
            <text x={x} y={H - 4} textAnchor="middle" fill="#8B929A" fontSize={10} fontFamily="'JetBrains Mono', monospace">
              {d.date}
            </text>
          </g>
        )
